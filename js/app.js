...
const I18N = {
  ar: {
    ...
    home: {
      ...
      editor: {
        title: 'تعديل المستند',
        hint: 'اختر النمط، ثم انقر على الصفحة لإضافة نص أو تظليل. للتوقيع: ارسم في المربع ثم «إضافة للصفحة». اسحب العناصر أو غيّر حجمها من الزوايا.',
        modeText: 'نص',
        modeHi: 'تظليل',
        modeSig: 'توقيع',
        textPh: 'اكتب النص ثم انقر على الصفحة',
        addSig: 'إضافة التوقيع للصفحة',
        clearSig: 'مسح اللوحة',
        prev: 'السابق',
        next: 'التالي',
        pageOf: 'صفحة {p} من {n}',
        download: 'تحميل PDF المعدّل',
        close: 'إنهاء / ملف جديد',
        viewOnly: 'عرض فقط (بدون تعديل)',
        needText: 'أدخل نصاً في الحقل أولاً',
        needSig: 'ارسم التوقيع أولاً',
        sigHint: 'وضع التوقيع: استخدم اللوحة بالأسفل ثم «إضافة للصفحة».',
        openFail: 'تعذّر فتح الملف للتعديل'
      }
    },
    tools: {
  ...
  en: {
    ...
    home: {
      ...
      editor: {
        title: 'Edit document',
        hint: 'Pick a mode, then click on the page to add text or a highlight. For signature: draw in the box, then «Add to page». Drag items or resize from corners.',
        modeText: 'Text',
        modeHi: 'Highlight',
        modeSig: 'Signature',
        textPh: 'Type text, then click on the page',
        addSig: 'Add signature to page',
        clearSig: 'Clear pad',
        prev: 'Previous',
        next: 'Next',
        pageOf: 'Page {p} of {n}',
        download: 'Download edited PDF',
        close: 'Close / new file',
        viewOnly: 'View only (no edit)',
        needText: 'Enter text in the field first',
        needSig: 'Draw your signature first',
        sigHint: 'Signature mode: use the pad below, then «Add to page».',
        openFail: 'Could not open file for editing'
      }
    },
    tools: {
...
function renderHome(main){
  ...
  main.innerHTML = `
    ...
    <div id="homeQuickEdit" class="home-quick-edit card hidden">
      <div class="home-editor-top">
        <h3 style="margin:0; font-size:1.15rem">${tt('home.editor.title')}</h3>
        <div class="home-editor-nav">
          <button type="button" class="nav-btn" id="homeEditPrev" aria-label="prev">${svg('chevDown')}</button>
          <span id="homeEditPageLbl"></span>
          <button type="button" class="nav-btn" id="homeEditNext" aria-label="next" style="transform:rotate(180deg)">${svg('chevDown')}</button>
        </div>
        <button type="button" class="nav-btn" id="homeEditViewOnly">${tt('home.editor.viewOnly')}</button>
        <button type="button" class="cancel-btn" id="homeEditClose">${tt('home.editor.close')}</button>
      </div>
      <p class="home-editor-hint">${tt('home.editor.hint')}</p>
      <div class="home-editor-toolbar">
        <label class="home-mode"><input type="radio" name="homeEditMode" value="text" checked> ${tt('home.editor.modeText')}</label>
        <label class="home-mode"><input type="radio" name="homeEditMode" value="highlight"> ${tt('home.editor.modeHi')}</label>
        <label class="home-mode"><input type="radio" name="homeEditMode" value="signature"> ${tt('home.editor.modeSig')}</label>
        <input type="text" id="homeEditTextInp" class="home-edit-text-inp" placeholder="${tt('home.editor.textPh')}">
      </div>
      <div id="homeEditSigPanel" class="home-edit-sig-panel hidden">
        <div class="signature-pad-wrap" dir="ltr">
          <canvas id="homeQuickSigCanvas"></canvas>
        </div>
        <div class="home-edit-sig-actions">
          <button type="button" class="nav-btn" id="homeEditClearSig">${tt('home.editor.clearSig')}</button>
          <button type="button" class="pick-btn" id="homeEditAddSig">${tt('home.editor.addSig')}</button>
        </div>
      </div>
      <div id="homeEditorStage" class="doc-stage home-editor-stage"></div>
      <div class="tool-actions" style="margin-top:16px">
        <button type="button" class="process-btn" id="homeEditDownload">${tt('home.editor.download')}</button>
      </div>
    </div>
    <div id="homeRest">
    <div class="badges">
  ...
  dz.ondrop = (e) => {
    ...
    if (file && isLikelyPdfFile(file) && file.size <= 10 * 1024 * 1024) openHomeQuickEditor(file);
...
/* ================== Home quick PDF editor (hero dropzone) ================== */
let homeEditFile = null;
let homeEditPagesData = [];
let homeEditItems = [];
let homeEditPageIx = 0;
let homeEditSigPad = null;

function disposeHomeEditSigPad() {
  if (homeEditSigPad && typeof homeEditSigPad.off === 'function') {
    try {
      homeEditSigPad.off();
    } catch (e) {
      /* ignore */
...
function initHomeEditSigPad() {
  const canvas = document.getElementById('homeQuickSigCanvas');
  if (!canvas || typeof SignaturePad === 'undefined') return;
  disposeHomeEditSigPad();
  const wrap = canvas.closest('.signature-pad-wrap');
  const maxW = wrap ? Math.min(Math.max(wrap.clientWidth - 16, 260), 520) : 400;
  ...
  canvas.width = w;
  canvas.height = h;
  homeEditSigPad = new SignaturePad(canvas, {
...
function updateHomeEditModeUI() {
  const mode = document.querySelector('input[name="homeEditMode"]:checked')?.value || 'text';
  const inp = document.getElementById('homeEditTextInp');
  const sig = document.getElementById('homeEditSigPanel');
  if (!inp || !sig) return;
  if (mode === 'signature') {
    inp.classList.add('hidden');
    sig.classList.remove('hidden');
    requestAnimationFrame(() => initHomeEditSigPad());
  } else {
...
function setHomeEditPageLabel() {
  const el = document.getElementById('homeEditPageLbl');
  if (!el) return;
  const n = homeEditPagesData.length || 1;
  const p = homeEditPageIx + 1;
  el.textContent = tt('home.editor.pageOf').replace('{p}', String(p)).replace('{n}', String(n));
}

function refreshHomeEditNavButtons() {
  const prev = document.getElementById('homeEditPrev');
  const next = document.getElementById('homeEditNext');
  if (prev) prev.disabled = homeEditPageIx <= 0;
  if (next) next.disabled = homeEditPageIx >= homeEditPagesData.length - 1;
}

function syncHomeEditItem(node) {
  const ann = homeEditItems.find(a => String(a.id) === node.id());
  if (!ann) return;
  ann.x = node.x();
  ann.y = node.y();
  ann.rotation = node.rotation();
  if (ann.type === 'highlight' || ann.type === 'image') {
    ann.width = node.width() * node.scaleX();
    ann.height = node.height() * node.scaleY();
  }
}

function renderHomeEditStage(pageIndex) {
  const stageEl = document.getElementById('homeEditorStage');
  if (!stageEl || !homeEditPagesData[pageIndex]) return;
  disposeHomeEditSigPad();
  stageEl.innerHTML = '';
  const img = document.createElement('img');
  img.src = homeEditPagesData[pageIndex];
  ...
  img.onload = () => {
    ...
    konvaStage.on('click tap', (e) => {
      ...
      const mode = document.querySelector('input[name="homeEditMode"]:checked')?.value;
      if (mode === 'signature') {
        toast(tt('home.editor.sigHint'), 'warn');
        return;
      }
      const pos = konvaStage.getPointerPosition();
      if (!pos) return;
      const textInp = document.getElementById('homeEditTextInp');
      const text = textInp?.value?.trim() || '';

      if (mode === 'text') {
        if (!text) {
          toast(tt('home.editor.needText'), 'warn');
          return;
        }
        homeEditItems.push({
          id: homeEditItems.length,
          pageIndex,
          type: 'text',
          value: text,
          x: pos.x,
      ...
      renderHomeEditStage(pageIndex);
    });

    homeEditItems.filter(a => a.pageIndex === pageIndex).forEach(a => {
      if (a.type === 'text') {
        ...
        addKonvaTransformers(textNode, konvaLayer, syncHomeEditItem);
        ...
        addKonvaTransformers(rectNode, konvaLayer, syncHomeEditItem);
      } else if (a.type === 'image') {
        Konva.Image.fromURL(a.value, (imageNode) => {
          imageNode.id(String(a.id));
          imageNode.setAttrs({
            x: a.x,
            y: a.y,
            width: a.width,
          ...
          addKonvaTransformers(imageNode, konvaLayer, syncHomeEditItem);
          konvaLayer.batchDraw();
        });
      }
    });
    konvaLayer.batchDraw();
    refreshHomeEditNavButtons();
    const mode = document.querySelector('input[name="homeEditMode"]:checked')?.value;
    if (mode === 'signature') {
      requestAnimationFrame(() => initHomeEditSigPad());
    }
...
async function openHomeQuickEditor(file) {
  if (typeof pdfjsLib === 'undefined' || !pdfjsLib.getDocument) {
    toast(
      LANG === 'ar'
        ? 'شغّل الموقع عبر خادم محلي ولا تفتح الملف مباشرة من القرص.'
        : 'Use a local server; do not open the HTML file directly from disk.',
      'error'
    );
    return;
  }
  ...
  try {
    ...
    for (let i = 1; i <= pdfDocJs.numPages; i++) {
      ...
      homeEditPagesData.push(canvas.toDataURL());
    }
    ...
    document.getElementById('homeRest')?.classList.add('hidden');
    document.getElementById('homeQuickEdit')?.classList.remove('hidden');

    const textInp = document.getElementById('homeEditTextInp');
    if (textInp) textInp.value = '';
    const textMode = document.querySelector('input[name="homeEditMode"][value="text"]');
    if (textMode) textMode.checked = true;
    updateHomeEditModeUI();
    setHomeEditPageLabel();
    renderHomeEditStage(0);
  } catch (err) {
    console.error(err);
    toast(tt('home.editor.openFail'), 'error');
  } finally {
    hideLoader();
  }
}

function closeHomeQuickEditor() {
  disposeHomeEditSigPad();
  homeEditFile = null;
  homeEditPagesData = [];
  homeEditItems = [];
  homeEditPageIx = 0;
  const stage = document.getElementById('homeEditorStage');
  if (stage) stage.innerHTML = '';

  document.getElementById('homeQuickEdit')?.classList.add('hidden');
  document.getElementById('homedz')?.classList.remove('hidden');
  document.getElementById('homeRest')?.classList.remove('hidden');
}

async function downloadHomeEditedPdf() {
  if (!homeEditFile) return;
  showLoader();
  try {
    const originalPdfDoc = await PDFLib.PDFDocument.load(await homeEditFile.arrayBuffer());
    const pages = originalPdfDoc.getPages();

    for (const ann of homeEditItems) {
      ...
      if (ann.type === 'text') {
        ...
        const img = await embedImageDataUrl(originalPdfDoc, ann.value);
        page.drawImage(img, {
          x: ann.x,
          y: page.getHeight() - ann.y - ann.height,
          width: ann.width,
    ...
    const fileName = await promptFileName(`edited-${homeEditFile.name}`);
...
function bindHomeQuickEditorUi() {
  document.querySelectorAll('input[name="homeEditMode"]').forEach((r) => {
    r.onchange = () => updateHomeEditModeUI();
  });
  const prev = document.getElementById('homeEditPrev');
  const next = document.getElementById('homeEditNext');
  if (prev) {
    prev.onclick = () => {
      if (homeEditPageIx <= 0) return;
      homeEditPageIx--;
      setHomeEditPageLabel();
      renderHomeEditStage(homeEditPageIx);
    };
  }
  if (next) {
    next.onclick = () => {
      if (homeEditPageIx >= homeEditPagesData.length - 1) return;
      homeEditPageIx++;
      setHomeEditPageLabel();
      renderHomeEditStage(homeEditPageIx);
    };
  }
  const homeEditCloseBtn = document.getElementById('homeEditClose');
  if (homeEditCloseBtn) homeEditCloseBtn.onclick = () => closeHomeQuickEditor();
  const homeEditViewBtn = document.getElementById('homeEditViewOnly');
  if (homeEditViewBtn) {
    homeEditViewBtn.onclick = () => {
      if (!homeEditFile) return;
      const f = homeEditFile;
      closeHomeQuickEditor();
      nav('viewer', { file: f });
    };
  }
  const homeEditDlBtn = document.getElementById('homeEditDownload');
  if (homeEditDlBtn) homeEditDlBtn.onclick = () => downloadHomeEditedPdf();
  const homeEditClrSig = document.getElementById('homeEditClearSig');
  if (homeEditClrSig) homeEditClrSig.onclick = () => homeEditSigPad && homeEditSigPad.clear();
  const homeEditAddSigBtn = document.getElementById('homeEditAddSig');
  if (homeEditAddSigBtn) homeEditAddSigBtn.onclick = () => {
    if (!homeEditSigPad || homeEditSigPad.isEmpty()) {
      toast(tt('home.editor.needSig'), 'warn');
      return;
    }
    const stageEl = document.getElementById('homeEditorStage');
    const stageBox = stageEl?.querySelector('img');
    const w = stageBox ? stageBox.clientWidth : 400;
    const h = stageBox ? stageBox.clientHeight : 560;
    const nw = 140;
    const nh = 70;
    homeEditItems.push({
      id: homeEditItems.length,
      pageIndex: homeEditPageIx,
      type: 'image',
      value: homeEditSigPad.toDataURL('image/png'),
      x: Math.max(8, w / 2 - nw / 2),
      y: Math.max(8, h / 2 - nh / 2),
      width: nw,
      height: nh,
      rotation: 0,
    });
    homeEditSigPad.clear();
    renderHomeEditStage(homeEditPageIx);
  };
}
...
function disposeSignaturePad() {
  if (signaturePad && typeof signaturePad.off === 'function') {
    try {
      signaturePad.off();
    } catch (e) {
      /* ignore */
...
function initSignaturePadOnCanvas() {
  const canvas = document.getElementById('signatureCanvas');
  if (!canvas || typeof SignaturePad === 'undefined') return;
  disposeSignaturePad();
  const wrap = canvas.closest('.signature-pad-wrap');
  const maxW = wrap ? Math.min(Math.max(wrap.clientWidth - 16, 280), 640) : 400;
  const w = Math.floor(maxW);
  const h = 200;
  canvas.style.width = `${w}px`;
  canvas.style.height = `${h}px`;
...
function renderSign(main) {
  const signHint = LANG === 'ar'
    ? '① اختر ملف PDF أولاً ② أضف التوقيع (رسم / كتابة / صورة) ③ اضغط تحميل. منطقة الرسم أدناه تعمل من اليسار لليمين.'
    : '① Choose a PDF first ② Add your signature (draw / type / image) ③ Download. The drawing area below is left-to-right for correct input.';
  renderToolLayout(main, 'sign', `
    ...
    <div id="signContainer" class="card hidden" style="margin-top:20px; padding:24px;">
      ...
      <p class="sub" style="margin:0 0 14px; color:var(--muted); font-size:14px; line-height:1.5;">${signHint}</p>
      <div class="sig-tabs">
  ...
  document.querySelectorAll('.sig-tab').forEach(tab => {
    tab.onclick = () => {
      ...
      if (tab.dataset.tab === 'draw') {
        requestAnimationFrame(() => initSignaturePadOnCanvas());
      }
    };
  });
  ...
  document.getElementById('saveSignature').onclick = () => {
    ...
    addPlacement(signaturePad.toDataURL('image/png'), 'image');
    signaturePad.clear();
  };
  requestAnimationFrame(() => initSignaturePadOnCanvas());
...
function renderNumber(main) {
  ...
  processNumberBtn.onclick = async () => {
    ...
    showLoader();
    try {
      let patternTpl = numberPatternInput.value || 'Page {p} / {n}';
      const sample = patternTpl.replace(/\{p\}/g, '1').replace(/\{n\}/g, '99');
      const latinOnly = /^[\x09\x0A\x0D\x20-\x7E]+$/;
      if (!latinOnly.test(sample)) {
        toast(
          LANG === 'ar'
            ? 'خط Helvetica لا يدعم العربية في الـ PDF. تم استخدام نمط لاتيني: Page {p} / {n}'
            : 'Standard PDF fonts only support Latin text. Using: Page {p} / {n}',
          'warn'
        );
        patternTpl = 'Page {p} / {n}';
      }
      ...
      for (let i = 0; i < totalPages; i++) {
        const page = pages[i];
        const { width, height } = page.getSize();
        const pageNumberText = patternTpl.replace(/\{p\}/g, String(i + 1)).replace(/\{n\}/g, String(totalPages));
