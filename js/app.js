...
// Full implementations are defined below
...
function renderCompress(main) {
  ...
  processCompressBtn.onclick = async () => {
    ...
    try {
      ...
      // Apply object stream compression based on selected quality level
      // Higher compression = more aggressive object stream usage
      const objectsPerTick = quality === 'high' ? 50 : quality === 'medium' ? 100 : 200;
      const pdfBytes = await pdfDoc.save({
        useCompression: true,
        objectsPerTick,
      });
...
function renderOrganize(main) {
  ...
  let organizeJsDoc = null; // pdfjsLib instance for thumbnail rendering
  ...
  async function loadOrganizePdf(file) {
    ...
    try {
      ...
      // Load separate pdfjsLib instance for rendering thumbnails
      organizeJsDoc = await pdfjsLib.getDocument({ data: await file.arrayBuffer() }).promise;
  ...
  async function renderOrganizedThumbs() {
    ...
    for (let i = 0; i < organizedPages.length; i++) {
      const pdfPage = await organizeJsDoc.getPage(organizedPages[i].index + 1); // Use pdfjsLib doc for rendering
...
function renderWordToPdf(main) {
  ...
  processWordToPdfBtn.onclick = async () => {
    ...
    try {
      ...
      const font = await pdfDoc.embedFont(PDFLib.StandardFonts.Helvetica);
      currentPage.drawText('Converted from Word (HTML preview below):', { x: 50, y: 750, font, size: 12 });
      ...
      for (const line of textLines) {
        ...
        currentPage.drawText(line.substring(0, 90), { x: 50, y: yPos, font, size: 10 });
        yPos -= 15;
      }
