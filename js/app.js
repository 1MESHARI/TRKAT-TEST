...
function renderHome(main){
  ...
  inp.onchange = (e) => {
    ...
    if (file && file.size <= 10 * 1024 * 1024) nav('viewer', {file});
...
function renderSplit(main){ renderToolLayout(main, 'split', \`<p>\${tt('common.loading')}</p>\`); }
function renderCompress(main){ renderToolLayout(main, 'compress', \`<p>\${tt('common.loading')}</p>\`); }
function renderOrganize(main){ renderToolLayout(main, 'organize', \`<p>\${tt('common.loading')}</p>\`); }
function renderRotate(main){ renderToolLayout(main, 'rotate', \`<p>\${tt('common.loading')}</p>\`); }
function renderWatermark(main){ renderToolLayout(main, 'watermark', \`<p>\${tt('common.loading')}</p>\`); }
function renderSign(main){ renderToolLayout(main, 'sign', \`<p>\${tt('common.loading')}</p>\`); }
function renderAnnotate(main){ renderToolLayout(main, 'annotate', \`<p>\${tt('common.loading')}</p>\`); }
function renderProtect(main){ renderToolLayout(main, 'protect', \`<p>\${tt('common.loading')}</p>\`); }
function renderUnlock(main){ renderToolLayout(main, 'unlock', \`<p>\${tt('common.loading')}</p>\`); }
function renderJpgToPdf(main){ renderToolLayout(main, 'jpgToPdf', \`<p>\${tt('common.loading')}</p>\`); }
function renderPdfToJpg(main){ renderToolLayout(main, 'pdfToJpg', \`<p>\${tt('common.loading')}</p>\`); }
function renderExtractText(main){ renderToolLayout(main, 'extractText', \`<p>\${tt('common.loading')}</p>\`); }
function renderCrop(main){ renderToolLayout(main, 'crop', \`<p>\${tt('common.loading')}</p>\`); }
function renderOcr(main){ renderToolLayout(main, 'ocr', \`<p>\${tt('common.loading')}</p>\`); }
function renderRepair(main){ renderToolLayout(main, 'repair', \`<p>\${tt('common.loading')}</p>\`); }
function renderNumber(main){ renderToolLayout(main, 'number', \`<p>\${tt('common.loading')}</p>\`); }
function renderWordToPdf(main){ renderToolLayout(main, 'wordToPdf', \`<p>\${tt('common.loading')}</p>\`); }
...
async function renderPage(num) {
  ...
  const renderCtx = { canvasContext: viewerCtx, viewport };
  ...
  renderToolLayout(main, 'split', `
    ...
    <div id="splitOptions" class="card hidden" style="margin-top:20px; padding:24px;">
      ...
      <div class="split-options">
        ...
        <div class="split-range-input hidden">
  ...
  processSplitBtn.onclick = async () => {
    ...
    try {
      ...
      if (splitType === 'all') {
        ...
        const rangeStr = splitRangeInput.value;
...
function renderCompress(main) {
  ...
  processCompressBtn.onclick = async () => {
    ...
    try {
      ...
      // This is a simplified compression. Real compression involves re-encoding images, fonts, etc.
      // For PDF-LIB, saving with a specific producer can sometimes reduce size, but true compression
      // is complex and often requires server-side tools or more advanced client-side libraries.
      const pdfBytes = await pdfDoc.save({
        useCompression: true,
        // The following options are more about optimization than direct 'quality' compression
        // For actual quality reduction (e.g., image downsampling), more advanced logic is needed.
        // This is a placeholder for demonstrating the tool's intent.
        ...(quality === 'low' && { add : true }),
        ...(quality === 'medium' && { add : true }),
        ...(quality === 'high' && { add : true }),
      });
...
function renderOrganize(main) {
  ...
  async function renderOrganizedThumbs() {
    ...
    for (let i = 0; i < organizedPages.length; i++) {
      ...
      const pdfPage = await pdfDoc.getPage(organizedPages[i].index + 1); // Use original pdfDoc for rendering
...
function renderSign(main) {
  ...
  // Uploaded Signature (Upload tab)
  document.getElementById('signatureImageUpload').onchange = (e) => {
    const file = e.target.files[0];
    if (file) {
  ...
  function renderSignStage(pageIndex) {
    ...
    img.onload = () => {
      ...
      // Redraw existing placements for this page
      placements.filter(p => p.pageIndex === pageIndex).forEach(p => {
        if (p.type === 'image') {
          Konva.Image.fromURL(p.value, (imageNode) => {
            ...
            addKonvaTransformers(imageNode);
  ...
  function addPlacement(value, type) {
    const pageIndex = 0; // For simplicity, always add to the first page for now
    const newPlacement = {
      id: placements.length, // Simple ID generation
      pageIndex,
      type,
      value,
    ...
    renderSignStage(pageIndex); // Re-render to show new placement
  }

  function addKonvaTransformers(node) {
    ...
    node.on('transformend', () => {
      // Update placement data after transform
      const p = placements.find(pl => pl.id === node.id());
      if (p) {
        p.x = node.x();
        p.y = node.y();
    ...
    node.on('dragend', () => {
      // Update placement data after drag
      const p = placements.find(pl => pl.id === node.id());
      if (p) {
        p.x = node.x();
        p.y = node.y();
    ...
    try {
      ...
      for (const placement of placements) {
        ...
        if (placement.type === 'image') {
          const img = await originalPdfDoc.embedPng(placement.value);
          page.drawImage(img, {
            x: placement.x,
...
function renderAnnotate(main) {
  ...
  function renderAnnotateStage(pageIndex) {
    ...
    img.onload = () => {
      ...
      // Add click listener to add annotations
...
function renderProtect(main) {
  ...
  processProtectBtn.onclick = async () => {
    ...
    try {
      const pdfDoc = await PDFLib.PDFDocument.load(await loadedPdf.arrayBuffer());
      const ownerPassword = ownerPasswordInput.value || undefined;

      const pdfBytes = await pdfDoc.save({
        use  : true,
        ownerPassword,
        userPassword,
        permissions: [PDFLib.CustomPermissions.Print, PDFLib.CustomPermissions.Copy, PDFLib.CustomPermissions.Edit, PDFLib.CustomPermissions.Modify],
      });
  ...
  function renderCropStage(pageIndex) {
    ...
    img.onload = () => {
      ...
      if (!cropRects[pageIndex]) {
        ...
        addKonvaTransformers(cropRects[pageIndex]);
      } else {
        konvaLayer.add(cropRects[pageIndex]);
        addKonvaTransformers(cropRects[pageIndex]);
      }
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        if (cropRect) {
          ...
          const newMediaBox = PDFLib.PDFArray.with = [
            PDFLib.PDFNumber.of(x),
            PDFLib.PDFNumber.of(page.getHeight() - y - height),
            PDFLib.PDFNumber.of(x + width),
            PDFLib.PDFNumber.of(page.getHeight() - y),
          ];
          page.setMediaBox(newMediaBox[0], newMediaBox[1], newMediaBox[2], newMediaBox[3]);
          page.setCropBox(newMediaBox[0], newMediaBox[1], newMediaBox[2], newMediaBox[3]);
        }
      }
...
function renderWordToPdf(main) {
  ...
  processWordToPdfBtn.onclick = async () => {
    ...
    try {
      ...
      // Placeholder for actual PDF generation from HTML
      const pdfDoc = await PDFLib.PDFDocument.create();
      const page = pdfDoc.addPage();
      const font = await pdfDoc.embedFont(PDFLib.StandardFonts.Helvetica);
      page.drawText('Converted from Word (HTML preview below):', { x: 50, y: 750, font, size: 12 });
      
      // This is a very basic representation. A real conversion would involve more sophisticated rendering.
      ...
      for (const line of textLines) {
        ...
        page.drawText(line, { x: 50, y: yPos, font, size: 10 });
        yPos -= 15;
      }
